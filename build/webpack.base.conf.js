...
module.exports = {
  ...
  resolve: {
    ...
    alias: {
      //后面的$符号指精确匹配，也就是说只能使用 import vuejs from "vue" 这样的方式导入vue.esm.js文件，
      //不能在后面跟上 vue/vue.js
  ...
  module: {
    rules: [
      ...
      },
      {
        ...
        // include选项指明这些目录下的文件要被babel-loader检测，还有一个exclude表示排除某些文件夹
      ...
      },
      {
        ...
        options: {
          ...
          // 3. 因为项目中会有动态引入而无法提前通过loader加载的图片，会如楼主后面说的，
          // 用CopyWebpackPlugin放到dist目录下。所以最后build完的图片资源就是两部分：一部分是dev下的整个
          // 图片文件夹（被复制了一份），另外的就是经过url-loader处理过的dist/img下的，带hash的图片
          name: utils.assetsPath('img/[name].[hash:7].[ext]')
          // 这个函数执行结果是 /img/[name].[hash:7].[ext]
          // 不知道吧 name 设置成 /img/[name].[hash:7].[ext] 
          //意欲何为，猜测应该是输出图片的路径或者是解析图片的路径
        }
      },
      {
